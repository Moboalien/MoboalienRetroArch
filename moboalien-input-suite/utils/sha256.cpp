...
// Minimal SHA256 implementation (public-domain like; simplified and self-contained)
// Note: This is a compact implementation adequate for configuration hashing.

namespace {

inline uint32_t rotr(uint32_t x, uint32_t n) { return (x >> n) | (x << (32 - n)); }

void sha256_transform(const uint8_t* chunk, uint32_t state[8]) {
...
std::array<uint8_t, 32> Sha256(const std::string& input) {
    ...
    size_t processed = 0;
    while (processed + 64 <= input.size()) {
        sha256_transform(reinterpret_cast<const uint8_t*>(input.data()) + processed, state);
        processed += 64;
    }

    // final block
    uint8_t block[64];
    size_t rem = input.size() - processed;
    std::memset(block, 0, 64);
    if (rem) std::memcpy(block, input.data() + processed, rem);
    block[rem] = 0x80;
